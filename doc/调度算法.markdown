...
1. 预处理策略 100%
2. 预选策略
    1. PodFitsResources 70%
    2. PodFitsHostPorts 70%
    4. PodMatchNodeSelector 70%
    5. PodToleratesNodeTaints 70%
    6. CheckNodeMemoryPressure 70%
    7. CheckNodeDiskPressure 70%
    8. MatchInterPodAffinity 70%
    9. PodExistingOnNode 70%
3. 优选策略
    1. RequestedPriority(Least and Most) 70%
    2. BalancedResourceAllocation 70%
    3. InterPodAffinityPriority ！
    4. SelectorSpreadPriority 70%
    5. NodeAffinityPriority 70%
    6. NodePreferAvoidPodsPriority 70%
    7. TaintTolerationPriority 70%
...
2. 从es读取节点的预测结果
3. 配置文件
4. 提供service,node下的pod列表，避免每次轮询所有pod。replace in path: "cache.getPodMap().values()"

## 还没拿到的数据
1. kube_node_status_allocatable下的指标，cpu core / memory byte / pods integer
2. node上已占用的什么协议的哪些端口；pod要占用哪些hostIP下的什么协议的哪些端口，或者 如果能拿到v1.Pod.Spec.Containers，读取container.Resources.Requests计算出pod要占用host的哪些端口
3. pod.Spec.Affinity, pod.Spec.Tolerations, NodeInfo.taints
