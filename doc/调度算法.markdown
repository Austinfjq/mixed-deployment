...
4. 并行优化
    1. 预选node->rule，node并行
    2. 优选rule->node，rule并行
5. 短期缓存维护
6. 消息队列接口
...
# 需要的数据 分类后
* 来自应用画像静态数据：
1. 应用的pod基本信息：需要的镜像名和镜像大小，需要占用的端口，需要占用资源cpu/mem/io，应用的pod是什么密集型
...
2. node已经存在的镜像和大小，node的资源cpu/mem/io容量。
