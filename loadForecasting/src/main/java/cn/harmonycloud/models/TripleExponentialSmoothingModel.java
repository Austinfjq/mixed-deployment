...
public class TripleExponentialSmoothingModel extends AbstractTimeBasedModel
{
    ...
    private int periodsPerYear = 0;  //每个周期数据的个数
        ...
        while ( it.hasNext() )
            {
                ...
                if ( dp.getTimeValue() > maxObservedTime )
                    maxObservedTime = dp.getTimeValue();
            }
        ...
        for ( int p=0; p<periodsPerYear*NUMBER_OF_YEARS; p++ )
            {
                ...
                double time = obs.getTimeValue();

                DataPoint dp = new DataPoint( trend , time );
                trendValues.add( dp );
            ...
            for ( int p=0; p<periodsPerYear; p++ )
                {
                    DataPoint dp = it.next();
                    index[p]
                        += (dp.getValue()/yearlyAverage[year])
                        / NUMBER_OF_YEARS;
                }
        ...
        for ( int p=0; p<periodsPerYear; p++ )
            {
                ...
                DataPoint obs = new DataPoint( index[p] ,time);
        ...
        throws IllegalArgumentException
    {
        ...
        DataPoint dp = new DataPoint( base , time );
        baseValues.add( dp );
        ...
        throws IllegalArgumentException
    {
        ...
        DataPoint dp = new DataPoint( index , time );
